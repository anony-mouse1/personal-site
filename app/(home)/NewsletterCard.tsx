...
export default function NewsletterCard() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setStatus("loading");
    try {
      // /api/subscribe writes to Supabase if it's configured, otherwise to a
      // local file. Either way the email is recorded before we say thanks.
  ...
  return (
    <article className="card p-6 md:p-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="font-sans text-[26px] font-bold leading-snug">Looking for free guides?</h2>
